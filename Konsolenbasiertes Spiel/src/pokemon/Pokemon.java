 ...
 * @author Kai
 *
 *	@class Pokemon Ein Trainer hat Pokemonn die Kaempfen koennen, dafuer haben sie Attacken ein Level und bestimmte Werte fuer den Kampf, je nach Anzahl von Exp die ein Pokemon insgesamt shon bekommen hat steigt es im Level bis max 100
 */
public class Pokemon {
	 ...
	 * @param maxkp int hoechst Anzahl der Kp die das Pokemon im vollkommen geheiltem Zustand hat
	 * @param kp int aktuelle Anzahl Kp
	 * @param staerke int Berechnungswert fuer Attacken auf andere Pokemon
	...
	private void zufallsWerteWild(int lvl){
		...
		for(int i=lvl;i>0; i--){
			...
			//sucht die aeltesten attacken, die das pokemon haette lernen koennen und die einem seiner typen entsprechen
			{
		...
		exp=(int)(aufstiegsgrenze(lvl)/5);//mache ich damit man als exp gewinn des trainers einfach die nehmen kann die das fremde pokemon hat so steigt es auch wenn die level hoeher gehen
		maxkp = (int)(lvl*2+20);
		kp=maxkp;
	...
	/**
	 * erhoeht die Exp des Pokemons um den Wert und initiiert intern einen Levelaufstieg wenn noetig
	...
	private double aufstiegsgrenze(int i){
		return 300*Math.pow(1.035, i);// ist eine Kurve die in etwa die benoetigen Exp gut bescheribt zunaechst eher wenig und gegen ende was mehr
	}
	...
	/**
	 * 
	 * @return Zeilenweise Ausgabe der Attacken mit Vorzahl um zu entscheiden welche man nutzen moechte
	 */
	...
	/**
	 * 
	 * @return Name, Attacken, Level, Exp, benoetigte Exp, wenn Level ueber 100: Name, Attacken und Level
	 */
	 ...
	 * @return Zeile in der Alle fuer den Kampf relevanten Daten enthalten sind
	 */
	...
	/**
	 * 
	 * @return Fangwert fuer das Spezielle Pokemon
	 */
	 ...
	 * @return Rueckgabe der Typen die ein Pokemon hat
	 */
	public Typ[] getTyp() {
		return typ;
	}
	...
	/**
	 * 
	 * @return Angriffswert fuer Kaempfe
	 */
	...
	/**
	 * 
	 * @return Verteigungswert fuer Kaempfe
	 */
	public int getVert() {
		return vert;
	}
	...
	/**
	 * 
	 * @return Schnelligkeit fuer Kaempfe
	 */
	public int getTempo() {
		return tempo;
	}
	...
	/**
	 * Gibt zurueck ob der Typ anfaellig gegen die Attacke ist
